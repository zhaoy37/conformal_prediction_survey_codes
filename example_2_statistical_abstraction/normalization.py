...
def normalization(x_vals,y_vals,x_hats,y_hats):
    alphas = []
    for i in range(len(x_vals[0])):
        R_vals = [math.sqrt((x_vals[j][i]-x_hats[j][i])**2 + (y_vals[j][i]-y_hats[j][i])**2) for j in range(len(x_vals))]
        alphas.append(1/(max(R_vals)))
    return alphas
...
def CpAndValidation(all_x, all_y, all_x_l, all_y_l, seed):
    ...
    alphas = normalization(x_calib_alphas, y_calib_alphas, x_l_calib_alphas, y_l_calib_alphas)
    time_end = time.time()
...
if __name__ == "__main__":
    ...
    for i in range(num_test_trials):
        seed = i*2
        alphas, D_cp, Ds_cp, CEC, one_time_count, time_cost = CpAndValidation(all_x, all_y, all_x_l, all_y_l, seed)
    ...
    print("Saving data.")
    with open("data_results/results_normalization.json", "w") as f:
        json.dump(results, f)
